...
const Sidebar = ({ isOpen, onClose }) => {
  ...
  const menuItems = [
    ...
    },
    {
      ...
      icon: (
        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 20h.01M12 12h.01M12 4h.01M21 20a9 9 0 11-18 0 9 9 0 0118 0z" />
        </svg>
      )
    },
    {
              ...
              <Link
                ...
                className={`flex items-center px-4 py-3 text-sm font-medium rounded-lg transition-colors ${
                  isActive
                    ...
                    : 'text-[var(--text-color)] hover:bg-[var(--primary-color)] hover:bg-opacity-10'
                }`}
              >
                <span className={isActive ? 'text-white' : 'text-[var(--text-color)]'}>
                  {item.icon}
                </span>
                <span className={`ml-3 ${isActive ? 'text-white' : 'text-[var(--text-color)]'}`}>
                  {item.name}
                </span>
                {isActive && (
